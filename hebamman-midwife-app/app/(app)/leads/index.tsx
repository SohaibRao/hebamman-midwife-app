    ...
    <View style={{ flex: 1, backgroundColor: COLORS.bg }}>
      ...
      {!midwifeId && pStatus === "loading" ? (
        ...
        <LeadList
          ...
          emptyText={
            tab === "upcoming" ? "No upcoming leads." : "No past leads."
          }
        />
      )}
        ...
        >
          <Pressable style={styles.overlay} onPress={() => setSelected(null)}>
            <Pressable style={styles.modalCard}>
              ...
              <Row
                label="Insurance"
                value={`${selected.insuranceType ?? "—"} (${
                  selected.insuranceCompany ?? "—"
                })`}
              />
              <Row label="Address" value={leadAddress(selected)} />
              <Row label="Client Status" value={selected.clientStatus} />
              <Row label="Appointment" value={selected.status} />
            </Pressable>
          </Pressable>
        </Modal>
      )}
...
function Row({ label, value }: { label: string; value?: string }) {
  return (
    <View style={{ flexDirection: "row", marginTop: 8, paddingHorizontal: 16 }}>
      <Text style={{ width: 110, color: COLORS.dim, fontWeight: "700" }}>
        {label}:
      </Text>
